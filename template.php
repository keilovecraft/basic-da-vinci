...
// Add Zen Tabs styles
if (theme_get_setting('dolphin_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'dolphin') .'/css/tabs.css', 'theme', 'screen');
}
...
function dolphin_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('dolphin_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = dolphin_id_safe('page-'. $path);
    $body_classes[] = dolphin_id_safe('section-'. $section);
...
function dolphin_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = dolphin_id_safe('node-type-' . $vars['type']);
  $vars['classes'] = implode(' ', $classes); // Concatenate with spaces
}

function dolphin_preprocess_comment_wrapper(&$vars) {
  $classes = array();
...
function dolphin_preprocess_comment(&$vars, $hook) {
...
function dolphin_menu_item_link($link) {
...
function dolphin_menu_local_tasks() {
  $output = '';
...
function dolphin_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
  ...
  $css_class = dolphin_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function dolphin_id_safe($string) {
 ...
 */
function dolphin_breadcrumb($breadcrumb) {
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('dolphin_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('dolphin_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('dolphin_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('dolphin_breadcrumb_title')) {
        if ($title = drupal_get_title()) {
          $trailing_separator = $breadcrumb_separator;
        }
      }
      elseif (theme_get_setting('dolphin_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
